...
class Jigowatt_reports extends Generic {
	...
	private function graphInit() {
		...
		if (!$this->end_date)   $this->end_date   = date('Ymd', strtotime('tomorrow'));
...
// 			$sql = "SELECT * FROM `login_integration` WHERE `$type` IS NOT NULL AND trim(`$type`) <> '';";
			$sql = "SELECT * FROM `login_users` WHERE `social_network_id` LIKE '$type%' <> '';";
		} else {
...
// 					date('Y-m-d', $this->start_date), date('Y-m-d', $this->end_date));
			$sql = "SELECT * FROM `login_users`
					WHERE `social_network_id` LIKE '$type%'
					AND `timestamp` >= '" . date('Y-m-d', $this->start_date) . "'
					AND `timestamp` <= '" . date('Y-m-d', $this->end_date) . "'
					ORDER BY `timestamp` ASC";
		}
