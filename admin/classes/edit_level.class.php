...
class Edit_level extends Generic {
	...
	private function retrieveInfo() {
		...
		if ($stmt->rowCount() < 1) :
			$this->error = _("Role doesn't exist!");
			return false;
		endif;
		...
		$this->options['level_name2'] = $this->options['level_name'];
	...
	private function validate() {
		...
		if(empty($this->options['level_name'])) {
			$this->error = _('You must enter a role name.');
			return false;
		}

		$params = array( ':level' => $this->options['level_name'] );
		$stmt   = parent::query("SELECT * FROM `login_levels` WHERE `level_name` = :level;", $params);

		if( $stmt->rowCount() > 0 && $this->options['level_name'] != $this->options['level_name2'] )
			$this->error = sprintf(_('Role name <b>%s</b> already exists'), $this->options['level_name']);
	...
	private function process($delete = false) {
		...
		if ($delete) :
			...
			if ($result[0] > 0) :
				$this->error = _("This role still has users in it!");
				return false;
			endif;

			$params = array( ':level' => $this->options['level_id'] );
			$stmt = parent::query("DELETE FROM `login_levels` WHERE `id` = :level;", $params);
			$this->result = sprintf(_('Role <b>%s</b> removed from database.'), $this->options['level_name']);
			...
			$stmt = parent::query("UPDATE login_levels SET level_name = :name, level_disabled = :disabled, redirect = :redirect, `welcome_email` = :welcome WHERE id = :id;", $params);
			$this->result = sprintf(_('Information updated for role <b>%s</b>.'), $this->options['level_name']);
