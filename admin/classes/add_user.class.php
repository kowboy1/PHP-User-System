...
class Add_user extends Generic {
	...
	public function validateInputAdmin() {
    ...
    if(isEmpty(filter_var($this->name, FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $patterns['name'])))) === true){
      $this->error .= '<li>'._('You must enter a valid name!').'</li>';
    }
    
    // ----------
    
    if(isEmpty(filter_var($this->username, FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $patterns['username'])))) === true){
    ...
    if(isEmpty(filter_var($this->email, FILTER_VALIDATE_EMAIL)) === true){
    ...
    if (isEmpty($this->error)) {
      ...
      parent::query("
        INSERT INTO `login_users` (`user_level`, `restricted`, `name`, `email`, `username`, `password`)
        VALUES (:user_level, '0', :name, :email, :username, :password);
        ",
        $params
      );
