...
class Edit_user extends Generic {
	...
	function __construct() {
		...
		/* Keep track of the original role in case there's a change. We might want to send emails on new role changes. */
	...
	private function validate() {
		...
		if ( $this->options['user_level'] != $this->original_level ) {
			...
			$sql = 'SELECT * FROM `login_levels` WHERE `welcome_email` = "1" AND `id` IN ('.$levels.')';
		...
		if(empty($this->options['name'])) {
			...
			$this->error = '<div class="alert alert-danger">'._('No user role has been selected.').'</div>';
		}
	...
	private function process() {
		...
		if(!empty($this->options['password'])) {
			$params = array(
				...
				':password' => hashPassword($this->options['password']),
				':id'       => $this->id
			);
		...
		if ( !empty($this->sendWelcome) ) :
			...
			if(!sendEmail($shortcodes['email'], $subj, $msg, $shortcodes))
