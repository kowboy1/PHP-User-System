...
if(isEmpty(filter_input(INPUT_POST, 'fbId', FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => SYSTEM_REGEX['anyNumericOptional']))))){
  $errors[] = _('Invalid facebook id!');
}
...
if (isEmpty($errors)) {
  ...
  $checkUserRegistrationQ = $dbh->query("
    SELECT *
    FROM login_users
    WHERE social_network_id = " . $dbh->quote('facebook_' . $_POST['fbId'])
  );
  ...
  if($checkUserRegistrationQ->rowCount() === 0){
    ...
    $query = $dbh->query("
      ...
      VALUES (
        ...
        '',
        " . $dbh->quote($_POST['name']) . ",
        " . $dbh->quote('facebook_' . $_POST['fbId']) . "
      )
    ");
